...
if __name__ == "__main__":
    ...
    def loss_fn(pred, target): return 0.4 * dice_loss(pred, target) + 0.4 * focal_loss(pred, target) + 0.2 * bce_loss(pred, target)

    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=5)

    best_iou = 0; patience = 7; no_improve = 0
    ...
    print(f"\nV1 Metrics saved to {REPORT_PATH}")
